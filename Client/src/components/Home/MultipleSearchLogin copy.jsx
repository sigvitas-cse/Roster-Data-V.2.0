...
const MultipleSearchLogin = () => {
  ...
  useEffect(() => {
    document.title = 'Login - Multiple Search | Triangle IP';
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        localStorage.setItem('userType', 'multiple');
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full p-3 pr-12 rounded-lg text-base bg-[#F8FAFC] text-[#1E293B] border border-[#CBD5E1] focus:outline-none focus:ring-2 focus:ring-[#38BDF8] focus:border-[#38BDF8] placeholder:text-[#64748B]"
                  />
                  <span
                    onClick={togglePasswordVisibility}
                    className={`fa ${showPassword ? 'fa-eye-slash' : 'fa-eye'} absolute top-1/2 right-4 -translate-y-1/2 cursor-pointer text-[#64748B] text-base hover:text-[#38BDF8] transition`}
                  />
                </div>
              </div>
...
export default MultipleSearchLogin;
