...
router.get("/FullDataAccess", async (req, res) => {
  const { query = '', page = 1, field } = req.query;
  const trimmed = query.trim();
  ...
  try {
    ...
    if (field === '_id' && trimmed) {
      // Fetch by _id if field is specified as _id
      const profile = await UserModel.findById(trimmed);
      results = profile ? [profile] : [];
      totalCount = profile ? 1 : 0;
    } else {
      ...
      const filter = query ? { $or: conditions } : {};
      results = await UserModel.find(filter).skip(skip).limit(limit);
